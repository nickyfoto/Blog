...
After fetch, before pull, you can use `git log` to view the difference.
...
$ git branch --no-merged # check which branch not merged
$ git branch -r --merged # check remote branch not merged
$ git branch --merged <branch_name> # go to branch_name, check commit history and see whether it includes current branch
$ git branch --merged origin/<branch_name> # check specific remote branch
$ git branch --merged <check_sum> # check with specific commit
```
...
```bash
$ git remote prune origin --dry-run # see what is will do before doing it
